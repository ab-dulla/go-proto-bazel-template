...
- **Containerization:** Docker (via `rules_docker`)
...
To build and push a service's Docker image to the configured container registry (`harbor.example.com`), use the `:push` target.
...
To push the base Helm chart as an OCI artifact to the registry, use the `:push-oci` target.
